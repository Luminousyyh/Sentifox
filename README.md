...
情感分析模型（`pytorch_model.bin`, ~33MB）未包含在代码仓库中。首次运行时会**自动下载**，或手动获取：

| 方式 | 命令 |
|------|------|
| **自动下载** | 首次运行 `app.py` 或 `cli.py` 时自动下载到 `models/sentiment/` |
| **手动下载** | 从 [Release 页面](https://github.com/Luminousyyh/sentifox/releases) 下载 `pytorch_model.bin`，放入 `models/sentiment/` |
| **HuggingFace** | `python -c "from transformers import AutoModel; AutoModel.from_pretrained('uer/roberta-base-finetuned-jd-binary-chinese')"` |

目录结构应为：
```
models/sentiment/
├── config.json
├── pytorch_model.bin    # <-- 33MB，需自行放置
└── vocab.txt
```
